...
def apply_boundary_refined_transform(config: dict[str, Any], transforms: dict[str, np.ndarray]) -> dict[str, Any]:
    refine_cfg = config.get("boundary_refinement", {})
    if not bool(refine_cfg.get("enabled", False)):
        return {"enabled": False}
    report_path = Path(refine_cfg["report_path"])
    report = read_json(report_path)
    model_name = str(refine_cfg.get("model") or report.get("model_selection", {}).get("selected_for_review", "affine"))
    if model_name not in report.get("models", {}):
        raise KeyError(f"Boundary refinement model {model_name!r} not found in {report_path}")
    refined = np.asarray(report["models"][model_name]["T_final_afm_to_ebsd"], dtype=np.float64)
    transforms["T_afm_reference_to_ebsd_grid_before_boundary_refine"] = transforms["T_afm_reference_to_ebsd_grid"].copy()
    transforms["T_afm_reference_to_ebsd_grid"] = refined
    return {
        "enabled": True,
        "report_path": str(report_path),
        "model": model_name,
        "matrix_direction": "AFM reference pixels -> EBSD grid coordinates (x=column, y=row)",
        "note": "Only EBSD spatial resampling uses this refined transform. AFM normal orientation still uses the AFM->SEM alignment rotation.",
        "T_final_afm_to_ebsd": refined.tolist(),
        "metrics": report.get("metrics", {}).get(model_name, {}),
    }
...
def save_3d_height_gold_png(path: Path, height_um: np.ndarray, scan_size_um: float, stride: int, dpi: int) -> None:
    ...
    ax.set_title("AFM 3D height used for surface-index display")
    ax.set_xlabel("x (um)")
    ax.set_ylabel("y (um)")
    ax.set_zlabel("height (um)")
...
def main() -> None:
    ...
    boundary_refinement_meta = apply_boundary_refined_transform(config, transforms)
    ...
    height_label = "AFM height reference, line-corrected" if bool(normals["line_correction"].get("enabled", False)) else "AFM height reference, no line correction"
    surface_overlay_label = "Surface index over AFM height reference"
    ...
    save_rgb(fig_dir / "01_afm_height_reference_line_corrected.png", height_rgb, height_label, dpi, marker=marker_afm)
    ...
    save_overlay(fig_dir / "11_surface_index_over_afm_height_reference.png", height_for_display, surface_rgb, valid, float(config["visualization"].get("surface_overlay_alpha", 0.82)), surface_overlay_label, marker_afm, dpi)
    ...
    metadata = {
        ...
        "registration": {
            ...
            "boundary_refinement": boundary_refinement_meta,
